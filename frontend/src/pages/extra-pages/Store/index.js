...
const rows = [
    {
        ...
        fullname: 'Tester',
        gender: 'men',
        birthday: '2022-06-06T03:40:40.847Z',
        role: 0,
        address: '123 Test'
    },
    {
...
export default function Staff() {
    ...
    const addOrEdit = async (item) => {
        if (item._id) {
            try {
                await axios.put(`/store/${item._id}`, item);
                await getData();
                setOpenPopup(false);
    ...
    const openInPopup = (item = null) => {
        setTitle(item !== null ? 'Sửa thông tin cửa hàng' : 'Tạo cửa hàng');
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const res = await axios.get(`/store?${query}`);
            setDataStore(res);
            setIsLoading(false);
        } catch (error) {
            setIsLoading(false);
    ...
    const emptyRows = page > 0 ? Math.max(0, (1 + page) * rowsPerPage - dataStore.length) : 0;
    useEffect(() => {
        getData();
    }, []);
                        ...
                        <MainCard>
                            <TableContainer>
                                <Table sx={{ minWidth: 750 }} aria-labelledby="tableTitle" size={dense ? 'small' : 'medium'}>
                                    ...
                                    />
                                    <TableBody>
                                        {stableSort(dataStore, getComparator(order, orderBy))
                            ...
                            <Popup title={title} openPopup={openPopup} setOpenPopup={setOpenPopup} onClose={() => setOpenPopup(false)}>
                                <StoreForm recordForEdit={recordForEdit} addOrEdit={addOrEdit} />
                            </Popup>
